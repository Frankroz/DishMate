  ...
  if (!article) {
    return `
      <div class="empty-message-container">
        <img src="/images/no-articles.png" alt="Article Not Found Icon"> <h2>Article not found!</h2>
...
class ArticleDetailManager {
  ...
  async init() {
    ...
    try {
      // Fetch all articles
      const allArticles = await this.externalServices.get(ARTICLES_JSON_URL);
      // Ensure 'allArticles' is an array, handle potential nesting if any
      ...
      const article = articlesArray.find((art) => {
        return art.id === parseInt(articleId);
      });
